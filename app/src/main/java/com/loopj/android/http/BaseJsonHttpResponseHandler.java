...
import go.fast.fastenvelopes.json.NoSerializeExclusionStrategy;
 ...
 * of abstract methods
 * {@link #onSuccess(int, Header[], String, Object)}
 * or
 * {@link #onFailure(int, Header[], Throwable, String, Object)}
...
public abstract class BaseJsonHttpResponseHandler<JSON_TYPE> extends
    ...
    public BaseJsonHttpResponseHandler(Class dataType) {
	this(DEFAULT_CHARSET);
	...
	if (statusCode != HttpStatus.SC_NO_CONTENT) {
		...
		public void run() {
		    try {
			final JSON_TYPE jsonResponse = parseResponseNow(
    ...
    public BaseJsonHttpResponseHandler<JSON_TYPE> setGson(Gson gson) {
	this.mGson = gson;
	return this;
    }

    protected JSON_TYPE parseResponseNow(String rawJsonData, boolean isFailure)
	    throws Throwable {
	Object obj = null;
	try {
